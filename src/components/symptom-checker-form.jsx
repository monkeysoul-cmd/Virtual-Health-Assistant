...
import { Loader2, AlertCircle, Sparkles, Stethoscope, FileText, User, PlusCircle, X, Award, MapPin, Phone, Calendar, } from 'lucide-react';
import { PrecautionaryAdvice } from './precautionary-advice';
import { TestSuggestions } from './test-suggestions';
import { Badge } from './ui/badge';
import { Separator } from './ui/separator';
import { doctors, commonSymptoms, commonConditions, conditionToSpecialty, } from '@/lib/data';
import { DoctorDetailsDialog } from './doctor-details-dialog';
function SubmitButton() {
    const { pending } = useFormStatus();
    return (<Button type="submit" disabled={pending} className="w-full md:w-auto">
...
export function SymptomCheckerForm() {
    ...
    return (<div className="w-full mx-auto space-y-8">
        ...
        <CardContent>
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              {commonSymptoms.map(symptom => (<Button key={symptom} variant="outline" size="sm" className="rounded-full" onClick={() => addSymptom(symptom)}>
          ...
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              {Object.keys(commonConditions).map(condition => (<Button key={condition} variant="outline" size="sm" className="rounded-full" onClick={() => addConditionSymptoms(condition)}>
                  <PlusCircle className="mr-2"/>
                  {condition}
                </Button>))}
            </div>
          ...
          <form action={formAction} className="space-y-4">
            <div className="relative">
              <Textarea name="symptoms" placeholder="I'm experiencing..." className="min-h-[120px] text-base pr-12" required value={symptoms} onChange={e => setSymptoms(e.target.value)}/>
              {symptoms && (<Button variant="ghost" size="icon" className="absolute right-2 top-2 h-8 w-8 rounded-full" onClick={() => setSymptoms('')} aria-label="Clear symptoms">
      ...
      {state.potentialConditions && state.potentialConditions.length > 0 && (<Card className="bg-card/80 backdrop-blur-sm shadow-lg animate-in fade-in-50 duration-500">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 font-headline text-2xl">
          ...
          <CardContent className="space-y-8">
            ...
            {mostProbableCondition && (<div className="space-y-6 rounded-lg border-2 border-primary bg-background/50 p-4 md:p-6 shadow-lg">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                   <h3 className="text-xl font-semibold">
                      Most Probable Condition
                    </h3>
                  <Badge variant="default" className="flex items-center gap-2 text-base px-3 py-1">
                    <Award className="w-5 h-5"/>
                    <span>{mostProbableCondition.likelihood}% Match</span>
                  </Badge>
                </div>
                <div>
                  <Badge variant="secondary" className="text-base font-medium px-4 py-2 capitalize">
                    {mostProbableCondition.condition}
                  </Badge>
                </div>
                <PrecautionaryAdvice conditions={[mostProbableCondition.condition]}/>
                <TestSuggestions conditions={[mostProbableCondition.condition]}/>
              </div>)}

            {/* Other Potential Conditions */}
            {otherConditions && otherConditions.length > 0 && (<div className="space-y-4">
                <Separator />
                <h3 className="text-xl font-semibold pt-4">Other Possibilities</h3>
                {otherConditions.map((item, index) => (<div key={index} className="space-y-4 rounded-lg border bg-background/50 p-4">
            ...
            <Card className="bg-primary/10 border-primary/50">
              ...
              <CardContent>
                {recommendedDoctor && (<Card key={recommendedDoctor.id} className="bg-card/80 backdrop-blur-sm shadow-lg mb-4 cursor-pointer transform hover:scale-105 transition-transform duration-300" onClick={() => setSelectedDoctorForBooking(recommendedDoctor)}>
      ...
      {selectedDoctorForBooking && (<DoctorDetailsDialog doctor={selectedDoctorForBooking} onClose={() => setSelectedDoctorForBooking(null)}/>)}
    </div>);
}
