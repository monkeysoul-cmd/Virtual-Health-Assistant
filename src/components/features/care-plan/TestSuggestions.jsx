...
import { testSuggestions } from '@/data/carePlans';
        ...
        {relevantTests.map((suggestion, index) =>
          suggestion.tests.map((test, testIndex) => {
            ...
            return (
              <Card
                key={`${index}-${testIndex}`}
                className="glass-card bg-slate-950/60 border border-emerald-500/20 hover:border-emerald-500/40 shadow-lg rounded-2xl overflow-hidden transition-all duration-200"
              >
